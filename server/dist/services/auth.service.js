import { AuthCodeType, CalendarDayStatus, } from '@prisma/client';
...
export async function loginStep1(login, password, fingerprint) {
    ...
    if (user.role === 'ADMIN' || trusted) {
        ...
        return { status: 'OK', token, user: sanitizeUser(user) };
    }
    ...
    await sendEmail(ADMIN_EMAIL, 'Смена устройства — код подтверждения', `Сотрудник ${user.fullName} (${user.login}) входит с нового устройства.\nКод для сотрудника: ${code}`);
    await prisma.trustedDevice.deleteMany({ where: { userId: user.id } });
    return { status: 'DEVICE_CHANGE', userId: user.id };
}
export async function loginStep2(userId, code, fingerprint) {
...
export async function setCalendarDayStatus(date, status, setById) {
...
export function bookingWhereForRole(role, pilotSimulators) {
    if (role === 'PILOT') {
