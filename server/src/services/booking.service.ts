import { BookingStatus, PaymentMethod, AuthCodeType } from '@prisma/client'
import { prisma } from '../lib/prisma.js'
import {
  addMinutesToTime,
  bookingConfirmCodeExpiresAt,
...
export async function createPublicBooking(input: {
  ...
  await sendEmail(
    input.email,
    'Код подтверждения бронирования Aviator',
...
export async function confirmPublicBooking(bookingId: string, code: string) {
  const booking = await prisma.booking.findUnique({ where: { id: bookingId } })
  if (!booking) throw new Error('NOT_FOUND')
  if (booking.status !== BookingStatus.PENDING_CONFIRMATION) throw new Error('INVALID_STATE')
...
export async function resendBookingCode(bookingId: string) {
  const booking = await prisma.booking.findUnique({ where: { id: bookingId } })
  ...
  await sendEmail(
    booking.email,
    'Новый код подтверждения бронирования Aviator',
    `Ваш код: ${code}`,
  )
